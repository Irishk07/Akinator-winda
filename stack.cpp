...
void SetHash(stack_t* stack) {
    ...
    stack->hash_data   = StackHash(stack->data, RealSizeStack(stack->capacity, CNT_CANARIES) * sizeof(stack_type_t));
...
type_error_t StackCtor(stack_t* stack, size_t start_capacity) { // TODO: test stack with other types than int
    ...
    stack->data = (stack_type_t*)calloc(RealSizeStack(stack->capacity, CNT_CANARIES), sizeof(stack_type_t));
...
type_error_t StackReSize(stack_t* stack, size_t old_capacity) {
    ...
    stack_type_t* temp_data = (stack_type_t*)my_recalloc(stack->data, RealSizeStack(stack->capacity, CNT_CANARIES) * sizeof(stack_type_t), 
                                                          RealSizeStack(old_capacity, CNT_CANARIES) * sizeof(stack_type_t));
...
type_error_t StackPush(stack_t* stack, stack_type_t new_value) {
...
type_error_t StackPeek(stack_t* stack, stack_type_t* peek_element) {
    type_error_t code_error = STACK_SUCCESS;
...
type_error_t StackPop(stack_t* stack, stack_type_t* deleted_value) {
    type_error_t code_error = STACK_SUCCESS;
